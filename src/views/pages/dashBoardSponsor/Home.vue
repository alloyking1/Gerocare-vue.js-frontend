    ...
    <div class="vx-row">
      ...
      <div class="vx-col w-full lg:w-2/5 mb-base">
        ...
        <div class="flex justify-between text-center mt-4" slot="no-body-bottom">
          <div
            class="w-1/2 border border-solid d-theme-border-grey-light border-r-0 border-b-0 border-l-0 border-t-0"
          >
            <p
              class="mb-4 text-2xl"
            >{{data.most_due_subscription.length === 0 ? "No due subscription" : data.most_due_subscription.patient.name}}</p>
            <p
              class="mt-4"
            >{{data.most_due_subscription.length === 0 ? 'No due subscription' : moment(data.most_due_subscription.created_at).format("DD-MM-YYYY") }}</p>
          </div>
          <div
            class="w-1/2 border border-solid d-theme-border-grey-light border-r-0 border-b-0 border-t-0"
          >
            <p
              class="mb-4 text-2xl font-danger"
            >{{ data.most_due_subscription.length === 0 ? 'No Visit count' : data.most_due_subscription.patient.visit_count}}</p>
            <p
              v-if="data.most_due_subscription.length != 0"
              class="mt-4 font-purple"
              @click="nextDueSubscription"
            >Pay Now</p>
          </div>
        </div>
      </div>
            ...
            <div class="p-8 clearfix">
              <div class="font-white">
                ...
                <h1>
                  ...
                  <span
                    class="font-white text-5xl"
                  >{{ formatWalletAmount(data.sponsor.wallet_balance)}}</span>
                </h1>
                <br />
                <small>
                  <router-link to="/billing">
                    <span class="font-purple">See billing history:</span>
                  </router-link>
                </small>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    fetchSponsor() {
      ...
      console.log(this.data.most_due_subscription.length);
    },

    async fetchAppointment() {
      await this.$store.dispatch(
        "fetchRecentVisit",
        this.$store.state.user.sponsor.id
      );
    },
