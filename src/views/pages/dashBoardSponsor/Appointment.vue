    ...
    <div class="vx-row">
      <div class="vx-col w-full sm:w-1/2 md:w-1/2 lg:w-1/4 xl:w-1/4 mb-base">
        ...
        <vx-card>
          ...
          <div
            class="upcoming-inner mb-3 p-2"
            v-for="(appointment, i) in upcomingAppointments"
            :key="i"
          >
            <small>
              Appointment with {{ appointment.doctor.name || "" }} on
              {{ moment(appointment.appointment_date).format("LLLL") }} for
              {{ appointment.patient.name }}.
            </small>
            <small v-if="isPreviousAppointmentEmpty"
              >You have no upcoming appointments</small
            >
          </div>
        </vx-card>
        <br />
